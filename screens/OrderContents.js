...
const OrderContents = ({route,navigation}) => {
    ...
    useEffect(()=>{
        ...
        firebase.firestore().collection("admin").doc(shopName).collection("customerDetail").doc(name).collection("orderList").onSnapshot((snapshot)=>setOrder(snapshot.docs.map((doc)=>doc.data())));
    ...
    return (<ScrollView>
        <View>
            ...
            {order.map(list=>(
               <Item name={list.name} brand={list.brand} quantity={list.quantity} price={list.price}/>
            ))}   
        </View>
        <View></View>
        <Button
